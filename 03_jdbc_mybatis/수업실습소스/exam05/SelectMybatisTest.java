...
public class SelectMybatisTest {
	...
	public static void main(String[] args) {
	   ...
	   */	
		...
		// mybatis는 기본적으로 auto commit 아니다.
		//insert
//		DeptDTO newDTO = new DeptDTO(1, "개발", "서울");
//		int n = session.insert("exam05.DeptMapper.save", newDTO);
//		session.commit(); // 명시적으로 commit 지정.
		
		//update
		DeptDTO updateDTO = new DeptDTO(1, "개발부", "서울시");
		int n = session.update("exam05.DeptMapper.update", updateDTO);
		session.commit();
		
		//delete
		int n2 = session.delete("exam05.DeptMapper.remove2", 1);
		session.commit();
