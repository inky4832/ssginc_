	...
	public static void main(String[] args) {
		...
		/*
	      ...
	      7. SqlSession의 메서드를 이용해서 Mapper의 태그 호출
	          ...
	          List<DeptDTO> list =  session.selectList("namespace.id값", 파라미터값); 
	          List<DeptDTO> list =  
	          session.selectList("namespace.id값", 파라미터값, new  RowBounds(시작값,갯수));  
	   ...
	   */	
		...
		List<DeptDTO> list2 = 
				session.selectList("exam04.DeptMapper.findAllOrder");
		...
		// offset은 0부터 시작
		List<DeptDTO> list3 = 
				session.selectList("exam04.DeptMapper.findAllPage",null,
						new RowBounds(3, 3));
